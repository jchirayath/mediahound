...
(() => {
  ...
  async function doUpload() {
    ...
    note.textContent = `Identifying ${ok} photo(s)… reading any barcode for an exact match — this can take a moment.`;
    // online=true so the pipeline can decode a barcode in the photo + resolve posters/metadata.
    fetch("api/rebuild", { method: "POST", headers: authHeaders(), body: JSON.stringify({ online: true }) })
  ...
  async function sha256(s) {
    // crypto.subtle exists only in a "secure context" (HTTPS or http://localhost). A self-hosted
    // server reached over plain HTTP on a LAN IP (e.g. a NAS at http://192.168.x.x:8080) is NOT a
    // secure context, so crypto.subtle is undefined there — fall back to a pure-JS SHA-256 so the
    // admin unlock still works. (Used only to gate the local editor UI, never for real secrets.)
    const bytes = new TextEncoder().encode(s);
    if (typeof crypto !== "undefined" && crypto.subtle && crypto.subtle.digest) {
      try {
        const buf = await crypto.subtle.digest("SHA-256", bytes);
        return [...new Uint8Array(buf)].map((x) => x.toString(16).padStart(2, "0")).join("");
      } catch (_) { /* fall through to the JS implementation */ }
    }
    return sha256Bytes(bytes);
  }
  // Minimal dependency-free SHA-256 (FIPS 180-4) over a byte array → lowercase hex.
  function sha256Bytes(bytes) {
    ...
    const total = (((l + 8) >> 6) + 1) << 6;            // padded length, multiple of 64
