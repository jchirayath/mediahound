# How ReelShelf compares

There are plenty of ways to catalog a movie collection. This is an honest look at the main
alternatives and where ReelShelf fits. *Facts verified June 2026 from each product's own site/repo;
...
keeps your catalog in **their cloud app** or a **dated desktop database**. ReelShelf takes a different
...
|---|---|---|---|---|---|---|
| **ReelShelf** | CLI → static website (any OS) | ✅ MIT | Free | **Photo of cover (OCR/AI)**, or manual | A static site you host (or open `index.html`) | ✅ Fully — plain files, host anywhere |
...
## What's genuinely different about ReelShelf

1. **Identify from photos, not barcodes.** Every other tool needs a UPC/EAN or that you type the
   title. ReelShelf reads the cover photo (Tesseract OCR by default, or Claude vision). This is the
...
2. **You get a real, modern website — that you own.** The open-source desktop tools (GCstar, Tellico,
   ...
   Java web app*. ReelShelf outputs a polished, responsive, searchable site you can host **free** on
...
3. **No subscription, no account, no cloud lock-in.** CLZ, Libib, and My Movies' best features live in
   their cloud. ReelShelf is offline-first and keeps everything in plain files you control.
...
## Where ReelShelf is *not* the best choice — pick the right tool
...
- **You want the fastest possible entry for barcoded discs and a slick mobile app** → **CLZ Movies**
  ...
  accurate for known UPCs; ReelShelf's photo identification is broader but needs a quick review pass.
...
## Honest limitations of ReelShelf today
...
- **Identification needs review** — OCR/AI can misread a cover; ReelShelf flags low-confidence ones for
...
barcode speed and a polished mobile app, ReelShelf is built exactly for you.
